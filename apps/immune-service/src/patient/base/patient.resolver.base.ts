...
export class PatientResolverBase {
  ...
  @common.UseInterceptors(AclFilterResponseInterceptor)
  @graphql.ResolveField(() => [Immune], { name: "immunes" })
  @nestAccessControl.UseRoles({
    resource: "Immune",
    action: "read",
    possession: "any",
  })
  async resolveFieldImmunes(
    @graphql.Parent() parent: Patient,
    @graphql.Args() args: ImmuneFindManyArgs
  ): Promise<Immune[]> {
    const results = await this.service.findImmunes(parent.id, args);
